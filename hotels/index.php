...
if (isset($child_age_json) && !empty($child_age_json))
  foreach ($child_age_json as $child_age_jsonK => $child_age_jsonV) foreach ($child_age_jsonV as $child_age_jsonVK => $child_age_jsonVV)
...
//$landmk=$database_hotel->query("select * from ps_landmark where selCityId=".$city_id)->fetchAll(PDO::FETCH_ASSOC);
//$landmarktxt=array_column($landmk,'txtLandmark');
//$landmarktxt=array_column($landmk,'txtLandmark');

// if(isset($_GET['landmark']))
// {
//   $lmark=ucwords(implode(' ',explode('-',$_GET['landmark'])));
// }
...
// if(isset($lmark)&&$lmark!='')
// {
//   if(array_key_exists($hotel_type,$landmk))
//   {
//     $filterHotelType=array($htl[$hotel_type]);
//   }
// }
// echo 'select c.name as cityname,cl.name as country, p.selLandmark,l.txtLandmark,p.offer_percentage,p.selPropertyTypeID, p.txtPropertyDescription, p.txtPropertyName,p.selStarRating,p.txtAddress1,p.txtAddress2,p.photo_gallery,p.selImages,r.id_property,r.id_room,r.period_from,r.period_to,rt.periodic_rateExtraBedAdult,rt.rateExtraBedAdult,rt.periodic_rateExtraBedChildMoreThanFive,rt.periodic_rateExtraBedChildLessThanFive,rt.rateExtraBedChildMoreThanFive,rt.rateExtraBedChildLessThanFive,rt.id_room_type from ps_property p left join ps_room r on(p.id_property = r.id_property and r.status=0) left join ps_landmark l on(l.id_landmark=p.selLandmark and l.status=0 and l.action=\'\') left join ps_room_type rt on(r.id_room = rt.id_room) left join ps_city c on(c.id_city=\''.$city_id.'\') left join ps_country_lang cl on(cl.id_country=c.id_country)  where 1 '.$countWhere.' and p.selCityId='.$city_id.' and p.status=0 and p.is_delete!=1';
// die;
$query = 'select rt.selMaxNoOfGuest, r.selRoomCategory,c.name as cityname,cl.name as country, p.selLandmark,l.txtLandmark,p.offer_percentage,p.selPropertyTypeID, p.txtPropertyDescription, p.txtPropertyName,p.selStarRating,p.txtAddress1,p.txtAddress2,p.photo_gallery,p.selImages,r.id_property,r.id_room,r.period_from,r.period_to,rt.periodic_rateExtraBedAdult,rt.rateExtraBedAdult,rt.periodic_rateExtraBedChildMoreThanFive,rt.periodic_rateExtraBedChildLessThanFive,rt.rateExtraBedChildMoreThanFive,rt.rateExtraBedChildLessThanFive,rt.id_room_type from ps_property p left join ps_room r on(p.id_property = r.id_property and r.status=0) left join ps_landmark l on(l.id_landmark=p.selLandmark and l.status=0 and l.action=\'\') left join ps_room_type rt on(r.id_room = rt.id_room) left join ps_city c on(c.id_city=\'' . $city_id . '\') left join ps_country_lang cl on(cl.id_country=c.id_country)  where 1 ' . $countWhere . ' and p.selCityId=' . $city_id . ' ' . $land_query . ' and p.status=0 and rt.status=0 and p.is_delete!=1 and rt.status=0';


$availableRoomsList = $database_hotel->query($query)->fetchAll(PDO::FETCH_ASSOC);
//print_r($availableRoomsList);
$cityname = $availableRoomsList[0]['cityname'];
...
$prpptyname = array_column($availableRoomsList, 'txtPropertyName');
$pptylandmark = array_unique(array_filter(array_column($availableRoomsList, 'txtLandmark')));
$prpptyname_slice = array_slice($prpptyname, 0, 5);



$jsonurl = "http://api.openweathermap.org/data/2.5/weather?q=" . strtolower($cityname) . ",in&APPID=1203cc2a7a2bac4c082860027b54b01c";
...
$weater_desc = $weather['weather'][0]['description'];
...
if (isset($availableRoomsList) && !empty($availableRoomsList)) {
  foreach ($availableRoomsList as $incKey => &$roomVal) {
    ...
    $without_rateValue = $without_rateValue + $without_extraRate;
    $roomVal['rate'] = $rateValue;
    $roomVal['without_rate'] = $without_rateValue;
...
<div>
          ...
          <div class="row" style="margin-top: 0;">
            <div class="bus_tickets">
              ...
              <div class="col-sm-12  col-md-12 col-lg-12 col-xs-12 checkins ">
                <form class="hotel_search" action="<?php echo $root_dir; ?>hotels/search/index.php" method="get"
                  id="hotel_search" style="display: none">
                  <div class="col-xs-12 col-sm-3">
                    <input type="text" class="form-control" required placeholder="City, Area or Hotel Name" id="to1"
                      autocomplete="off" name="goingto"
                      value="<?php echo (isset($city_name) && !empty($city_name) ? $city_name : ''); ?>" />
                    <input type="hidden" id="to_hidden1" name="to_hidden"
                      value="<?php echo (isset($city_id) && !empty($city_id) ? $city_id : ''); ?>">
                  </div>
                  ...
                  <div class="col-xs-12 col-sm-2 ">
                    ...
                    <input type="text" class="form-control" placeholder="Select date" id="hotels_month" name="checkin"
                      value="<?php echo (isset($check_in) && !empty($check_in) ? $check_in : ''); ?>">
                  </div>
                  <div class="col-xs-12 col-sm-2 ">
                    <i class="fa fa-calendar" aria-hidden="true"></i>
                    <input type="text" class="form-control" placeholder="Select date" id="hotels_month1" name="checkout"
                      value="<?php echo (isset($check_out) && !empty($check_out) ? $check_out : ''); ?>">
                  </div>
                  ...
                  <div class="col-xs-12 col-sm-2 " style="margin-bottom: 10px;">
                    <div class="section_room" style="position: relative">
                      ...
                      <div id="button_add_roo" type="button" data-toggle="collapse" data-target="#demo"><span
                          class="total_guest_room">1 Room, 2 Guests</span><span style="float: right;"><i
                            class="fa fa-angle-down arrows_addroom" aria-hidden="true"
                            style="position: absolute;top: 10px;"></i></span></div>
                      ...
                      <div class="collapse room_res2 " style="" id="demo">
                          ...
                          <div class="col-sm-12 pax_container" data="1">
                            <div class="row" style="margin-top: 4px;">
                              <div class="col-sm-3 detail_pax detail_pax_1">
                                <p class="detail_pax_p" style="font-family: 'Roboto',sans-serif;">Room <span
                                    class="roomnumber">1
                                  </span>
                                </p>
                              ...
                              <div class="col-sm-8 content_pax content_pax_1" style="margin-top: -20px;">
                                ...
                                <div class="row">
                                  <div class="col-sm-5 col-xs-6">
                                    ...
                                    <select class="adultlist adultlist_1 actives"
                                      onchange="changeadult(this.value,this)">
                                  ...
                                  <div class="col-sm-6 col-xs-6">
                                    ...
                                    <select class="childlist childlist_1 actives"
                                      onchange="changechild(this.value,this)">
                        ...
                        <button class="rooms_in_hotel " data="2" type="button" onclick="addroom(this)"
                          style="font-family: 'Roboto',sans-serif;background: transparent!important;color: #dd3236!important;border:none!important;margin-top: -10px;padding: 5px 10px!important;font-size: 11px!important;font-weight: 500!important;">+
                          Add Room</button>
                        <span class="done" onclick="ondone()"
                          style="color:#ad1f1f; font-family: 'Roboto',sans-serif;margin-right: 2px;">Done</span>
                      </div>
                    </div>
                ...
                <div class="display_lables">
                  <div class="col-md-3 label_cont">
                    <?php echo (isset($city_name) && !empty($city_name) ? $city_name : ''); ?>
                  </div>
                  <div class="col-md-2 label_cont">
                    <?php echo (isset($check_in) && !empty($check_in) ? date('D , d  M ', strtotime($check_in)) : ''); ?>
                  </div>
                  <div class="col-md-2 label_cont">
                    <?php echo (isset($check_out) && !empty($check_out) ? date('D , d  M ', strtotime($check_out)) : ''); ?>
                  </div>
                  <div class="col-md-2 label_cont">
                    <?php echo $text; ?>
                  </div>
                  <div class="col-md-2 label_cont">
                    <button type="button" class="btn-cus-submit modify_tag btn modify_before_Act"
                      onclick="enable_forsearch()" style="position: relative;"><span
                        style="text-decoration: none!important;position: absolute;left: 0">+</span>Modify Search
                    </button>
                  </div>
                </div>
  ...
  <div class="container full_conta " id="full_conta_sty">
    ...
    <div class="hicontent_index" style="">


      <span class="count_holt red">We Found
        <?php echo count($hotelListArr) . '</span> <h1 style="float: left;">' . $text1 ?> in
        <?php echo ucwords($city_name) ?>
        </h1>
        ...
        <li>
          <span style="padding: 9px 0px;">
            ...
            if ($details[0]['country']) { ?>
              <a class="values_cityname" href="<?php echo $url_country; ?>">Hotels in
                <?php echo $details[0]['country']; ?> /
              </a>
            <?php
            }
            ?>
            <a class="values_cityname" href="<?php echo $url_destination; ?>">
              <?php echo ucwords($hotel_type); ?> in
              <?php echo $city_name; ?>
            </a>
            <?php
            if (trim($_GET['star']) != 'hotels') {
              ...
              if ($_GET['star'] != '') {
                ...
                ?>/ <a class="values_cityname1" href="<?php echo $url_category; ?>" src="">
                  <?php echo $text1; ?>
                </a>
              <?php }
            } ?>
        ...
        <aside id="secondary" class="left-sidebar widget-area one-fourth" role="complementary">
                ...
                <div class="column">
                  ...
                  <div class="where rooms ">
                    <div class="hotelde_inns">
                      ...
                      <h2 style="font-size: 15px;" class="hotel_found"><span class="count_amount">
                          <?php echo count($hotelListArr); ?>
                        </span> Hotels found
                        <?php echo $text1; ?> in <span>
                          <?php echo $city_name; ?>
                        </span>
                      </h2>
                      <h6>Landmarks </h6>
            ...
            <li class="widget widget-sidebar">
              <article class="refine-search-results byt_search_widget BookYourTravel_Search_Widget">
                <div class="dropdown col-xs-12">
                  ...
                  <h2 class="show_hide_type">
                    <?php echo $text1; ?> In
                    <?php echo ucwords($_GET['city']) ?>
                  </h2><Span class="show_atag_hotel" href="#hotelTypes" data-toggle="collapse">Show</Span>
                  <hr>
                  <div id="hotelTypes" class="collapse">
                    <ul class="dropdown-menus new_drop_menu ">
                      ...
                      <li><a href="#"><input type="checkbox" data-sort-type="hoteltype" data-value="1" <?php echo in_array(1, $filterHotelType) ? 'checked' : ''; ?>>
                          ...
                          <label>Hotels In
                            <?php echo ucwords($_GET['city']) ?><span style="float: right;color: #adabab;">
                              <?php echo isset($count_property_type[1]) ? $count_property_type[1] : ''; ?>
                            </span>
                          </label></a>
                      </li>
                      ...
                      <li><a href="#"><input type="checkbox" data-sort-type="hoteltype" data-value="2" <?php echo in_array(2, $filterHotelType) ? 'checked' : ''; ?>>
                          ...
                          <label>Resort In
                            <?php echo ucwords($_GET['city']) ?><span style="float: right;color: #adabab;">
                              <?php echo isset($count_property_type[2]) ? $count_property_type[2] : ''; ?>
                            </span>
                          </label></a></li>
                      ...
                      <li><a href="#"><input type="checkbox" data-sort-type="hoteltype" data-value="3" <?php echo in_array(3, $filterHotelType) ? 'checked' : ''; ?>>
                          ...
                          <label>Apartment In
                            <?php echo ucwords($_GET['city']) ?><span style="float: right;color: #adabab;">
                              <?php echo isset($count_property_type[3]) ? $count_property_type[3] : ''; ?>
                            </span>
                          </label></a></li>
                      ...
                      <li><a href="#"><input type="checkbox" data-sort-type="hoteltype" data-value="4" <?php echo in_array(4, $filterHotelType) ? 'checked' : ''; ?>>
                          ...
                          <label>Villa In
                            <?php echo ucwords($_GET['city']) ?><span style="float: right;color: #adabab;">
                              <?php echo isset($count_property_type[4]) ? $count_property_type[4] : ''; ?>
                            </span>
                          </label></a></li>
                      ...
                      <li><a href="#"><input type="checkbox" data-sort-type="hoteltype" data-value="5" <?php echo in_array(5, $filterHotelType) ? 'checked' : ''; ?>>
                          ...
                          <label>Homestay In
                            <?php echo ucwords($_GET['city']) ?><span style="float: right;color: #adabab;">
                              <?php echo isset($count_property_type[5]) ? $count_property_type[5] : ''; ?>
                            </span>
                          </label></a></li>
          ...
          <ul>
            <li class="widget widget-sidebar">
              <article class="refine-search-results byt_search_widget BookYourTravel_Search_Widget">
                <div class="dropdown col-xs-12 ">
                  <h2>Price </h3><Span class="show_atag_hotel" href="#hotelPrice" data-toggle="collapse">Show</Span>
                    <hr>
                    <div id="hotelPrice" class="collapse">
                      <ul class="dropdown-menus new_drop_menu ">
                        ...
                        <li><a href="#">
                            ...
                            <label> Upto <i class="fa fa-rupee"></i> 999 <span class="filter_number price_range_0_999">
                                <?php echo $count_price['0-999'] ?>
                              </span> </label></a></li>

                        <li><a href="#"><input type="checkbox" data-sort-type="price" data-value="1000#3000"><label> <i
                                class="fa fa-rupee"></i> 1000 to <i class="fa fa-rupee"></i> 3000 <span
                                class="filter_number price_range_1000_3000">
                                <?php echo $count_price['1000-3000'] ?>
                              </span></label></a></li>

                        <li><a href="#"><input type="checkbox" data-sort-type="price" data-value="3000#5000"><label> <i
                                class="fa fa-rupee"></i> 3000 to <i class="fa fa-rupee"></i> 5000 <span
                                class="filter_number price_range_3000_5000">
                                <?php echo $count_price['3000-5000'] ?>
                              </span></label></a></li>

                        <li><a href="#"><input type="checkbox" data-sort-type="price" data-value="5000#10000"><label> <i
                                class="fa fa-rupee"></i> 5000 to <i class="fa fa-rupee"></i> 10000 <span
                                class="filter_number price_range_5000_10000">
                                <?php echo $count_price['5000-10000'] ?>
                              </span></label></a></li>
                      </ul>
                    </div>
                </div>
            ...
            <li class="widget widget-sidebar">
              <article class="refine-search-results byt_search_widget BookYourTravel_Search_Widget">
                <div class="dropdown col-xs-12 ">
                  ...
                  <h2>
                    <?php echo $text1 ?> in
                    <?php echo ucwords($destination); ?> By Star Rating
                  </h2>
                  ...
                  <div id="hotelStar" class="collapse">
                    ...
                    <ul class="dropdown-menus new_drop_menu checkbox_design" style="line-height: 2px;">
                      <li><a href="#"><input <?php echo (isset($star) && $star == 1) ? 'checked' : ''; ?> type="checkbox"
                            id="1_star" data-sort-type="rating" data-value="1">
                          <label for="1_star">
                            <!-- <i class="fa fa-star-o" aria-hidden="true"></i> -->
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span class="filter_number star_rating_1">
                              <?php echo $count_star['1']; ?>
                            </span>

                          </label></a></li>

                      <li><a href="#"><input <?php echo (isset($star) && $star == 2) ? 'checked' : ''; ?> type="checkbox"
                            id="2_star" data-sort-type="rating" data-value="2">
                          <label for="2_star">
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span class="filter_number star_rating_2">
                              <?php echo $count_star['2']; ?>
                            </span>
                          </label></a></li>

                      <li><a href="#"><input <?php echo (isset($star) && ($star == 3)) ? 'checked' : ''; ?> type="checkbox"
                            id="3_star" data-sort-type="rating" data-value="3">
                          <label for="3_star">
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span class="filter_number star_rating_3">
                              <?php echo $count_star['3']; ?>
                            </span>
                          </label></a></li>

                      <!--<li class="divider"></li>-->
                      <li><a href="#"><input <?php echo (isset($star) && $star == 4) ? 'checked' : ''; ?> type="checkbox"
                            id="4_star" data-sort-type="rating" data-value="4">
                          <label for="4_star">
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span class="filter_number star_rating_4">
                              <?php echo $count_star['4']; ?>
                            </span>
                          </label></a></li>

                      <li><a href="#"><input <?php echo (isset($star) && $star == 5) ? 'checked' : ''; ?> type="checkbox"
                            id="5_star" data-sort-type="rating" data-value="5">
                          <label for="5_star">
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span><i class="material-icons">&#xE838;</i></span>
                            <span class="filter_number star_rating_5">
                              <?php echo $count_star['5']; ?>
                            </span>
                          </label></a></li>
                    </ul>
                  </div>
          ...
          <article class="refine-search-results byt_search_widget BookYourTravel_Search_Widget">
                      <div class="dropdown col-xs-12" style="overflow: hidden;">
                          ...
                          <h2>Popular Areas In <?php echo ucwords($_GET['city']) ?> </h3>
            ...
            <li class="widget widget-sidebar">
              <article class="refine-search-results byt_search_widget BookYourTravel_Search_Widget">
                <div class="dropdown col-xs-12">
                  ...
                  <Span class="show_atag_hotel" href="#from_flight" data-toggle="collapse">Show</Span>
            ...
            <li class="widget widget-sidebar">
              <article class="refine-search-results byt_search_widget BookYourTravel_Search_Widget">
                <div class="dropdown col-xs-12">
                  ...
                  <h2>
                    <?php echo ucwords($_GET['city']) ?> Map
                  </h2>
                  ...
                  <div id="map_current" class="collapse">
                    ...
                    <iframe id="map" width="600" height="450" frameborder="0" style="border:0;width:100%;height: 300px;"
        ...
        <section class="three-fourth">
          ...
          <div class="">
            ...
            <div class="dropdown col-sm-12"
              style="padding: 15px 0;margin-bottom: 10px; box-shadow: 0 0 30px rgba(0, 0, 0, 0.1)!important;">

              <div class="col-sm-5  ">
                <div style="border: 1px solid #ccc;  position: relative;">

                  <input style="color:#555!important;font-weight: normal;" type="button"
                    value="Select by Address, Landmark and Location " class="searchforadd">
              ...
              <div style="padding: 0 15px ">
                ...
                <div class="remove_landmarks">
                  <?php echo $locatio_address; ?>
                </div>
      ...
      -->
          ...
          <div class="sort-by col-sm-12">
            ...
            <ul class="sorting_ul">
              ...
              <li>Price<span class="caretidv"><i class="fa fa-caret-down price_sort"
                    onclick="sortfor('price_sort','fa-caret-down',1);" aria-hidden="true"></i><i
                    class="fa fa-caret-up price_sort" aria-hidden="true"
                    onclick="sortfor('price_sort','fa-caret-up',0);"></i></span></li>
              <li>Stars <span class="caretidv"><i class="fa fa-caret-down star_sort"
                    onclick="sortfor('star_sort','fa-caret-down',1);" aria-hidden="true"></i><i
                    class="fa fa-caret-up star_sort" aria-hidden="true"
                    onclick="sortfor('star_sort','fa-caret-up',0);"></i></span></li>
              <li>Rating <span class="caretidv"><i class="fa fa-caret-down rating_sort"
                    onclick="sortfor('rating_sort','fa-caret-down',1);" aria-hidden="true"></i><i
                    class="fa fa-caret-up rating_sort" aria-hidden="true"
                    onclick="sortfor('rating_sort','fa-caret-up',0);"></i></span></li>
          ...
          <div class="deals">
            ...
            <div class="">
              ...
              if (isset($hotelListArr) && !empty($hotelListArr)) {
                  ...
                  <article class="accommodation_item full-width ">
                    ...
                    <figure class="assuerd_div">
                      <a href="" title="Villa Maria">
                        <img
                          src="<?php echo (isset($hotels['photo_gallery']) && !empty($hotels['photo_gallery']) ? $hotels['photo_gallery'] : $root_dir . 'hotels/search/images/noimage.jpg'); ?>"
                          alt="Villa Maria" />
                    ...
                    <div class="details">
                      ...
                      <span class='rating'>
                        ...
                        <br>
                        <a style="margin-top:10px;cursor: pointer;display: block;color:#ccc;"> <span
                            style="color:#bb0f0f;">100</span> Reviews <i class="fa fa-angle-right"
                      ...
                      <div class="price">
                        Price from
                        <?php
                        if ($hotels['offer_price']) { ?>
                          <span class="price_span">
                            <em>

                              <span class="price_strike_span"><i class="fa fa-inr" aria-hidden="true"></i>
                                <?php echo $hotels['price'] ?>
                              </span>
                            </em>
                      ...
                      <div class='description '>
                        ...
                        <p class="discription_overflow  data_<?php echo $count; ?>">
                          <?php //substr(("fdiusgodfug"),0,100)
                          ...
                          <?php if (strlen($pag_ct) >= 150) { ?>
                            <a class="more_infos" data="<?php echo $count; ?>">More info</a>
                          <?php }
                              } ?>
                        ...
                        <div class=" icons_all">
                          <a href="#" data-toggle="tooltip" title="Room Service">
                            <i class="material-icons" <?php if (!in_array('room_service', $hotels['amenities']) && !in_array('room_service_24_hours', $hotels['amenities']))
                              echo 'style="color:#c8c8c8;cursor:not-allowed;"'; ?>>room_service</i>
                          </a>
                          <a href="#" data-toggle="tooltip" title="Gym / Spa">
                            <i class="material-icons" <?php if (!in_array('exercise_gym', $hotels['amenities']) && !in_array('spa', $hotels['amenities']))
                              echo 'style="color:#c8c8c8;cursor:not-allowed;"'; ?>>fitness_center</i>
                          </a>
                          <a href="#" data-toggle="tooltip" title="Swimming Pool">
                            <i class="material-icons" <?php if (!in_array('swimming_pool', $hotels['amenities']))
                              echo 'style="color:#c8c8c8;cursor:not-allowed;"'; ?>>pool</i>
                          </a>
                          <a href="#" data-toggle="tooltip" title="Wi-fi">
                            <i style="font-size: 22px;" class="fa fa-wifi" aria-hidden="true" <?php if (!in_array('internet_access_in_rooms', $hotels['amenities']))
                              echo 'style="color:#c8c8c8;cursor:not-allowed;font-size: 22px;position: relative; "'; ?>></i>
                          </a>
                          <a href="#" data-toggle="tooltip" title="Restaurant">
                            <i class="material-icons" <?php if (!in_array('restaurant', $hotels['amenities']))
                              echo 'style="color:#c8c8c8;cursor:not-allowed;"'; ?>>restaurant</i>
                          </a>
                          <a href="#" data-toggle="tooltip" title="Internet Access">
                            <i class="material-icons" <?php if (!in_array('internet', $hotels['amenities']))
                              echo 'style="color:#c8c8c8;cursor:not-allowed;"'; ?>>desktop_windows</i>
                          </a>
                        </div>
                      </div>
                      <div class='actions'>
                        <form class="book-hotel-form" name="hotel_book"
                          action="<?php echo $root_dir ?>hotels/search/book.php">
                          ...
                          <button type="submit" class=" btn btnBook1 select_room_btn" name="hotel_book"
                            value="Book Now">Select Room</button>
                        </form>
                      </div>
                    </div>
                ...
                <div class="col-md-12 inner_first_content1">
                  <div class=" text-center"><label>No Results Found</label></div>
                </div>
              <?php } ?>
          ...
          <div style="box-shadow: 1px 1px 6px #ccc;padding: 13px 21px 34px;">
            <h4 style="text-align: center;padding: 10px 0;color: #dd3236;">
              <?php echo $text1 ?> in
              <?php echo ucwords($destination); ?>
            </h4>
            <div class="seo_content_index">
              <div class="seocontent_div">

                <p>
                  <?php echo ucwords($_GET['city']) ?> is a one of beautiful destination in
                  <?php echo ucwords($city_name) ?>
                </p>
                <p> we have </p>
                <h2>
                  ...
                  <h2>, <h3>
                      ...
                      <?php echo ucwords($_GET['city']) ?>
                      <h3>, <h2>
                          <?php echo implode(', ', $attractions_hjf); ?>,
                          <?php echo $count_property_type[$filterHotelType[0]]; ?>
                          <?php echo ucwords($hotel_type); ?>
                          <h2> near <h2>
                              <?php echo implode(', ', $pptylandmark); ?>
                            </h2>, <h3>you Can get upto 50% offer for each Booking ,find More
                              <?php echo $text1; ?>
                            </h3>
                            <h2>
                              <?php echo implode(', ', $prpptyname_slice); ?>
                            </h2>

                            <p>
                              <?php echo $weater_desc; ?>
                            </p>
                            <p>Destination weather :
                              <?php echo $celcius; ?><sup>.</sup>Celcius
                            </p>
                            Destination weather Min Temp :
                            <?php echo $temp_min; ?>
                            Destination weather Max Temp :
                            <?php echo $temp_max; ?>
