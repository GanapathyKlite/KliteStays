...
$availableRoomsList = $database_hotel->query('select rt.photo_gallery as photo_gall,rt.selMaxNoOfGuest as selMaxNoOfGuests,rt.minNoOfGuest as minNoOfGuests, p.txtPhone,p.txtMobile,p.txtPropertyDescription,l.txtlandmark,p.selPropertyTypeID,p.offer_percentage,p.txtNoOfGuestRooms,p.txtPropertyName,p.selStarRating,p.txtAddress1,p.txtAddress2,p.txtZip,p.selImages,p.txtLatitude,p.txtLongitude,p.rateLunchAdult,p.rateLunchMoreThanFive,p.rateLunchLessThanFive,p.rateDinnerAdult,p.rateDinnerMoreThanFive,p.rateDinnerLessThanFive,p.txtCGST,p.txtSGST,p.txtTAC,p.cancellation_policy_3_days,p.cancellation_policy_3_days_more,p.terms_and_conditions,r.*,r.txtPropertyDescription as roomDescription,rt.id_room_type,rt.rateExtraBedAdult,rt.rateExtraBedChildMoreThanFive,rt.rateExtraBedChildLessThanFive,rt.periodic_rateExtraBedAdult,rt.periodic_rateExtraBedChildMoreThanFive,rt.periodic_rateExtraBedChildLessThanFive,rt.txtRoomName ,cl.name as countryName, s.name as stateName, c.name as cityName from ps_property p left join ps_room r on(p.id_property = r.id_property and r.status=0) left join ps_room_type rt on(r.id_room = rt.id_room) left join ps_country_lang cl on(p.selCountryId = cl.id_country) left join ps_state s on(p.selStateId = s.id_state) left join ps_city c on(p.selCityId = c.id_city) left join ps_landmark l on(l.id_Landmark=p.selLandmark and l.status=0 and l.action=\'\') where 1'.$countWhere.' and p.id_property = '.$id_property.' and p.status=0')->fetchAll();
...
//echo '<pre>';print_r($availableRoomsList);echo '</pre>';
					foreach($availableRoomsList as $room){ ?>
                        ...
                        <img src="<?php echo $room['imagesRoom'][0]?>" alt="Standard Double room" /><a href="<?php echo $room['imagesRoom'][0]?>" class="image-overlay" rel="prettyPhoto[gallery206]"></a> 
                        <?php }else{ ?>
