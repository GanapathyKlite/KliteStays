...
{
  if(strpos($_GET['keyword'],'percent'))
  {
    $keyword_fromseo=ucwords(implode(' ',explode('-',str_replace("percent","%",$_GET['keyword']))));
  }else
  {
    $keyword_fromseo=ucwords(implode(' ',explode('-',$_GET['keyword'])));
...
$qry='select rt.photo_gallery as photo_gall,rt.selMaxNoOfGuest as selMaxNoOfGuests,rt.minNoOfGuest as minNoOfGuests, p.txtPhone,p.txtMobile,p.txtPropertyDescription,l.txtlandmark,p.selPropertyTypeID,p.offer_percentage,p.txtNoOfGuestRooms,p.txtPropertyName,p.selStarRating,p.txtAddress1,p.txtAddress2,p.txtZip,p.selImages,p.txtLatitude,p.txtLongitude,p.rateLunchAdult,p.rateLunchMoreThanFive,p.rateLunchLessThanFive,p.rateDinnerAdult,p.rateDinnerMoreThanFive,p.rateDinnerLessThanFive,p.txtCGST,p.txtSGST,p.txtTAC,p.cancellation_policy_3_days,p.cancellation_policy_3_days_more,p.terms_and_conditions,r.*,r.txtPropertyDescription as roomDescription,rt.id_room_type,rt.rateExtraBedAdult,rt.rateExtraBedChildMoreThanFive,rt.rateExtraBedChildLessThanFive,rt.periodic_rateExtraBedAdult,rt.periodic_rateExtraBedChildMoreThanFive,rt.periodic_rateExtraBedChildLessThanFive,rt.txtRoomName ,cl.name as countryName, s.name as stateName, c.name as cityName from ps_property p left join ps_room r on(p.id_property = r.id_property and r.status=0) left join ps_room_type rt on(r.id_room = rt.id_room) left join ps_country_lang cl on(p.selCountryId = cl.id_country) left join ps_state s on(p.selStateId = s.id_state) left join ps_city c on(p.selCityId = c.id_city) left join ps_landmark l on(l.id_Landmark=p.selLandmark and l.status=0 and l.action=\'\') where 1'.$countWhere.' and p.id_property = '.$id_property.' and p.status=0 and rt.status=0';

$availableRoomsList = $database_hotel->query($qry)->fetchAll();
...
$text1=ucwords($text1);
if(isset($selPropertyTypeID)&&$selPropertyTypeID!='')
{
  if(in_array($selPropertyTypeID,$htl))
  {
    $hotel_type=array_search($selPropertyTypeID, $htl);
  }
}
...
$roomRateArr = $propertyRooms = $hotelListArr = $price = $roomNames = $gstArrSelect = $withoutgstArrSelect= array();
	...
	foreach($availableRoomsList as $incKey => &$roomVal){
		...
		if(isset($roomTypeArr) && !empty($roomTypeArr)){
			...
			foreach($roomTypeArr as $roomTypeKey => $roomTypeVal){
				...
				$isbreakfast=explode(',',$roomTypeVal['is_breakfast']);
				//if(in_array(1,$isbreakfast))
				//{
				//	if($roomTypeVal['is_breakfast'] == 1)
					$roomVal['is_breakfast'] =$isbreakfast;
					...
					$without_masterRate[0] = $roomTypeVal['withoutbreakfast_rate_sun'];
					$without_masterRate[1] = $roomTypeVal['withoutbreakfast_rate_mon'];
					$without_masterRate[2] = $roomTypeVal['withoutbreakfast_rate_tue'];
					$without_masterRate[3] = $roomTypeVal['withoutbreakfast_rate_wed'];
					$without_masterRate[4] = $roomTypeVal['withoutbreakfast_rate_thu'];
					$without_masterRate[5] = $roomTypeVal['withoutbreakfast_rate_fri'];
					$without_masterRate[6] = $roomTypeVal['withoutbreakfast_rate_sat'];

				/*}
				if(in_array(2,$isbreakfast))
				{
					//echo '56567';
					if((int)$roomTypeVal['is_breakfast'] ==2)
					$roomVal['is_breakfast'] =2;
					//echo '<pre>';print_r($roomVal);	echo '</pre>';
					//array_push($availableRoomsList, "trtertrtrt");
					//array_push($availableRoomsList, $roomVal);
				}*/
			...
			$dateV	= date('d',strtotime($dateValue));
				...
				if(isset($periodicRateList) && !empty($periodicRateList))
					foreach($periodicRateList as $periodicRateKey => $periodicRateVal){
						...
						$without_periodicRate[0] = $periodicRateVal['withoutbreakfast_periodic_rate_sun'];
						$without_periodicRate[1] = $periodicRateVal['withoutbreakfast_periodic_rate_mon'];
						$without_periodicRate[2] = $periodicRateVal['withoutbreakfast_periodic_rate_tue'];
						$without_periodicRate[3] = $periodicRateVal['withoutbreakfast_periodic_rate_wed'];
						$without_periodicRate[4] = $periodicRateVal['withoutbreakfast_periodic_rate_thu'];
						$without_periodicRate[5] = $periodicRateVal['withoutbreakfast_periodic_rate_fri'];
						$without_periodicRate[6] = $periodicRateVal['withoutbreakfast_periodic_rate_sat'];
					...
					if(isset($without_periodicRate[date('w',strtotime($dateValue))]) && !empty($without_periodicRate[date('w',strtotime($dateValue))]))
						$without_rateValue += $without_periodicRate[date('w',strtotime($dateValue))];
					else if(isset($without_masterRate[date('w',strtotime($dateValue))]) && !empty($without_masterRate[date('w',strtotime($dateValue))]))
						$without_rateValue += $without_masterRate[date('w',strtotime($dateValue))];
					
		}

		$extraRate =$without_extraRate= 0;
		if(isset($guest) && !empty($guest)){
		foreach($guest as $guestkey => $guestval){
			//echo '<pre>';print_r($roomVal);
				if($guestkey>1) $extraRate += $rateValue;
				if($guestkey>1) $without_extraRate += $without_rateValue;
					...
					foreach($daterange as $dateK => $date){
						...
						if($dateValue >= $roomVal['period_from'] && $dateValue <= $roomVal['period_to'] && isset($roomVal['periodic_rateExtraBedAdult']) && !empty($roomVal['periodic_rateExtraBedAdult']))
							{ 
								...
								$without_extraRate += $extraAdult * $roomVal['periodic_rateExtraBedAdult'];
							}
						else
						{

							$extraRate += $extraAdult * $roomVal['rateExtraBedAdult'];
							$without_extraRate += $extraAdult * $roomVal['rateExtraBedAdult'];
						}
					}
				}
				if($guestval['\'child\''] > 0){
					...
					foreach($daterange as $dateK => $date){
						...
						if($dateValue >= $roomVal['period_from'] && $dateValue <= $roomVal['period_to']){
							for($extraAdultInc=0; $extraAdultInc<$extraAdult; $extraAdultInc++){
								...
								{
									$extraRate += $childRate;
									$without_extraRate += $childRate;
								}
								else
								{

									$extraRate += $childRate1;
									$without_extraRate += $childRate1;
								}
							}
						}else{
							for($extraAdultInc=0; $extraAdultInc<$extraAdult; $extraAdultInc++){
			...
			{
				if($guestval['\'adult\''] > $roomVal['selMaxNoOfGuests']){ // > 2
					$extraAdult = $guestval['\'adult\''] - $roomVal['selMaxNoOfGuests']; // - 2;
					foreach($daterange as $dateK => $date){
						$dateValue = $date->format("Y-m-d");
		...
		$without_rateValue=round($without_rateValue);
		//echo $without_rateValue;
		//echo $baseTariff;
		 if($offer_percentage!=''&&$offer_percentage!=0)
           {
				...
				$bf=($without_rateValue/(100+$offer_percentage))*100;     
				$offerrate=round($without_rateValue-$bf);  
				$without_rateValue =round($without_rateValue-$offerrate);
				
           }
		$roomVal['baseTariff'] =  $rateValue;
		$roomVal['without_baseTariff'] =$without_rateValue;
		...
		if($roomVal['without_baseTariff'] > 999 && $roomVal['without_baseTariff'] <= 2499){
			$roomVal['without_cgstAmount'] = $withoutgstArrSelect[$roomVal['id_room'].'_'.$roomVal['id_room_type']]['cgstAmount'] = round(($roomVal['without_baseTariff']*($hotelConfig['ps_hotel_pay_cgst_2499']/100))*count($guest));
			$roomVal['without_sgstAmount'] = $withoutgstArrSelect[$roomVal['id_room'].'_'.$roomVal['id_room_type']]['sgstAmount'] = round(($roomVal['without_baseTariff']*($hotelConfig['ps_hotel_pay_sgst_2499']/100))*count($guest));
		}else if($roomVal['without_baseTariff'] > 2499 && $roomVal['without_baseTariff'] <= 7499){
			$roomVal['without_cgstAmount'] = $withoutgstArrSelect[$roomVal['id_room'].'_'.$roomVal['id_room_type']]['cgstAmount'] = round(($roomVal['without_baseTariff']*($hotelConfig['ps_hotel_pay_cgst_7499']/100))*count($guest));
			$roomVal['without_sgstAmount'] = $withoutgstArrSelect[$roomVal['id_room'].'_'.$roomVal['id_room_type']]['sgstAmount'] = round(($roomVal['without_baseTariff']*($hotelConfig['sps_hotel_pay_sgst_7499']/100))*count($guest));
		}else if($roomVal['without_baseTariff'] > 7499){
			$roomVal['without_cgstAmount'] = $withoutgstArrSelect[$roomVal['id_room'].'_'.$roomVal['id_room_type']]['cgstAmount'] = round(($roomVal['without_baseTariff']*($hotelConfig['ps_hotel_pay_cgst_7500']/100))*count($guest));
			$roomVal['without_sgstAmount'] = $withoutgstArrSelect[$roomVal['id_room'].'_'.$roomVal['id_room_type']]['sgstAmount'] = round(($roomVal['without_baseTariff']*($hotelConfig['ps_hotel_pay_sgst_7500']/100))*count($guest));
		}else{
			$roomVal['without_cgstAmount'] = $withoutgstArrSelect[$roomVal['id_room'].'_'.$roomVal['id_room_type']]['cgstAmount'] = 0;
			$roomVal['without_sgstAmount'] = $withoutgstArrSelect[$roomVal['id_room'].'_'.$roomVal['id_room_type']]['sgstAmount'] = 0;
		}
		...
		if(isset($without_rateValue) && !empty($without_rateValue))
			$without_rateValue = (count($guest) > 1 ? round(($without_rateValue + $without_extraRate) / count($guest)) : round($without_rateValue + $without_extraRate));
		$roomVal['without_rate'] = $roomVal['without_baserate'] = (isset($without_rateValue) ? $without_rateValue : 0);
		...
		if(isset($without_rateValue) && !empty($without_rateValue)){
			if(!isset($propertyRooms[$roomVal['id_property']]['without_rate']) || (isset($propertyRooms[$roomVal['id_property']]['without_rate']) && $without_rateValue < $propertyRooms[$roomVal['id_property']]['without_rate'])){
				$propertyRooms[$roomVal['id_property']]['without_rate'] = $without_rateValue;
			}
		}
		...
		if($roomVal['without_rate']!=0)
		{
			//$price[$roomVal['id_room'].'_'.$roomVal['id_room_type']] = round($roomVal['without_rate']);
			 $price1[$roomVal['id_room'].'_'.$roomVal['id_room_type']][2] = round($roomVal['without_rate']);

		}else if($roomVal['rate']!=0)
		{
			//$price[$roomVal['id_room'].'_'.$roomVal['id_room_type']] = round($roomVal['rate']);
			$price1[$roomVal['id_room'].'_'.$roomVal['id_room_type']][1] = round($roomVal['rate']);
		}
		$roomVal['rate'] = round($roomVal['rate']);
		$roomVal['without_rate'] = round($roomVal['without_rate']);
		
	}
}
$photo_gallery = array();
$custompricearr=array();
foreach($price1 as $ksy=>$val)
{

	if($val[1]!=0&&$val[1]!=''||$val[0]!=0&&$val[0]!='')
	{
		$custompricearr[$ksy]=$val[1];
	}elseif($val[2]!=0&&$val[2]!='')
	{
		$custompricearr[$ksy]=$val[2];
	}

}
if(isset($custompricearr) && !empty($custompricearr))
	array_multisort($custompricearr, SORT_ASC, $availableRoomsList);
...
foreach ($availableRoomsList as $keyIte => $row)
{
	...
	$breakfastarr=explode(',',$breakfastResult[0][0]);

   foreach($breakfastarr as $keys=>$vals)
   {
      if($vals==1||$vals==0)
      {
         $roomNames[$row['id_room'].'_'.$row['id_room_type']][1] = ucwords($row['txtRoomName']).(isset($roomTypes[$row['id_room_type']]) && !empty($roomTypes[$row['id_room_type']]) ? ' - '.$roomTypes[$row['id_room_type']] : '').' With Breakfast';

      }elseif($vals==2)
      {
          $roomNames[$row['id_room'].'_'.$row['id_room_type']][2] = ucwords($row['txtRoomName']).(isset($roomTypes[$row['id_room_type']]) && !empty($roomTypes[$row['id_room_type']]) ? ' - '.$roomTypes[$row['id_room_type']] : '').' Without Breakfast';
      }
	...
	//print_r($row['photo_gall']);
	if(isset($row['photo_gall']) && !empty($row['photo_gall']) && $row['photo_gall'] != ',,,'){

		$room_gallery_arr = json_decode($row['photo_gall'],true);
		//print_r($room_gallery_arr);
		if(isset($room_gallery_arr) && !empty($room_gallery_arr)){
			foreach($room_gallery_arr as $room_gallery_arr_val){
				//print_r($room_gallery_arr_val);
				$availableRoomsList[$keyIte]['imagesRoom'][] = _BO_HOTEL_IMG_DIR_.'rooms/'.$row['id_room'].'/'.$row['id_room_type'].'/'.current($room_gallery_arr_val);
				...
				if(!isset($availableRoomsList[$keyIte]['imageRoom']) || empty($availableRoomsList[$keyIte]['imageRoom']))
					$availableRoomsList[$keyIte]['imageRoom'] = _BO_HOTEL_IMG_DIR_.'rooms/'.$row['id_room'].'/'.$row['id_room_type'].'/'.current($room_gallery_arr_val);
...
<div class="main" role="main" id="primary">
   ...
   <div class="wrap">
   	<h1 style="text-align: center;"><?php echo ucwords($title);?></h1>
      <!--main content--><!--breadcrumbs-->
      <nav role="navigation" class="breadcrumbs">
         <ul>
            <li><a href="index.php" title="Home">Home / </a></li>
            <li><a href="<?php echo $root_dir;?><?php echo strtolower($cityNameMap);?>/<?php echo $hotel_type;?>-in-<?php echo $destination; ?>" title="Hotels"><?php echo $city_name; ?></a></li>
            ...
            <section id="availability" class="tab-content initial">
               <article>
                  ...
                  <div class='text-wrap availability_text'>
                  	<?php 
                  	$room_category=array_filter(array_column($availableRoomsList, 'txtRoomName'));
                  ...
                  //  echo '<pre>';print_r($availableRoomsList);  echo '</pre>';
					...
					foreach($availableRoomsList as $room){ 
						//print_r($room['is_breakfast']);
						if(!empty($room['is_breakfast']))
						{
							foreach($room['is_breakfast'] as $key=>$value)
							{
								if($value==1)
								{
									?>
<li id="room_type_206">
                        ...
                        <figure class="left">
                        	<?php  if(!empty($room['imagesRoom'])){ ?>
                        <img src="<?php echo $room['imagesRoom'][0]?>" alt="Standard Double room" /><a href="<?php echo $room['imagesRoom'][0]?>" class="image-overlay" rel="prettyPhoto[pp_gal]"></a> 
                        <?php }else{ ?>
                        ...
                        <div class="meta room_type">
                           <h3><p style="font-size: 19px;">
                           	<?php echo (isset($room['txtRoomName']) && !empty($room['txtRoomName']) ? ucwords($room['txtRoomName']).(isset($roomTypes[$room['id_room_type']]) && !empty($roomTypes[$room['id_room_type']]) ? ' ('.$roomTypes[$room['id_room_type']].')' : '') : 'Taj Club, Room, Business Lounge Access, City View'); ?></p></h3>
                           <div class='text-wrap room_meta'>
                        		<p>Breakfast Included. Non refundable.</p> 
                           </div>
                           <a href='javascript:void(0)' class='more-info text_color'  title='+ more info'>+ more info</a>						
                        </div>
				 ...
				 <input style="color:#fff;margin-top: 35px!important;" class="btn button_color  book_room" type="button" value="Select Room" style="margin-top: 10px;" data-rate="<?php echo (isset($room['rate']) && !empty($room['rate']) ? $room['rate'] : ''); ?>" data-base-rate="<?php echo (isset($room['baserate']) && !empty($room['baserate']) ? $room['baserate'] : ''); ?>" data-id-room="<?php echo (isset($room['id_room']) && !empty($room['id_room']) ? $room['id_room'] : ''); ?>" data-id-room-type="<?php echo (isset($room['id_room_type']) && !empty($room['id_room_type']) ? $room['id_room_type'] : ''); ?>" data-is-breakfast="1"   data-basetarrif="<?php echo $room['baseTariff']; ?>" data-cgstamount="<?php echo $room['cgstAmount']; ?>" data-sgstamount="<?php echo $room['sgstAmount']; ?>" data-basetarrif="<?php echo $room['baseTariff']; ?>"> </div>
                        ...
                        <div class="meta room_type">
                           <h3><p style="font-size: 19px;">
                           	<?php echo (isset($room['txtRoomName']) && !empty($room['txtRoomName']) ? ucwords($room['txtRoomName']).(isset($roomTypes[$room['id_room_type']]) && !empty($roomTypes[$room['id_room_type']]) ? ' ('.$roomTypes[$room['id_room_type']].')' : '') : 'Taj Club, Room, Business Lounge Access, City View'); ?></p></h3>
                           <div class='text-wrap room_meta'>
                           <p>Breakfast Not Included. Non refundable.</p>
                           </div>
                           <a href='javascript:void(0)' class='more-info text_color'  title='+ more info'>+ more info</a>						
                        </div>
                        <div class="room-information">
                           ...
                           <div>
                              ...
                              <em>
                              <span class="curr"><i class="fa fa-rupee"></i> </span> 
                              <span class="amount">   <?php echo (isset($room['without_baseTariff']) && !empty($room['without_baseTariff']) ? $room['without_baseTariff'] : ''); ?></span>
                              </em>
				 ...
				 <input style="color:#fff;margin-top: 35px!important;" class="btn button_color  book_room" type="button" value="Select Room" style="margin-top: 10px;" data-rate="<?php echo (isset($room['without_rate']) && !empty($room['without_rate']) ? $room['without_rate'] : ''); ?>" data-base-rate="<?php echo (isset($room['without_baserate']) && !empty($room['without_baserate']) ? $room['without_baserate'] : ''); ?>" data-id-room="<?php echo (isset($room['id_room']) && !empty($room['id_room']) ? $room['id_room'] : ''); ?>" data-id-room-type="<?php echo (isset($room['id_room_type']) && !empty($room['id_room_type']) ? $room['id_room_type'] : ''); ?>" data-is-breakfast="2"  data-cgstamount="<?php echo $room['without_cgstAmount']; ?>" data-sgstamount="<?php echo $room['without_sgstAmount']; ?>" data-basetarrif="<?php echo $room['without_baseTariff']; ?>"> </div>
               ...
               }else
               {
				 ...
				 ?>
                     <div class="price">
                        ...
                        <span id="totalRateSpan" class="text_color">           	
                        ...
                        <a type="button" class="text_color" data-toggle="modal" data-target="#myModalFareDetails" style="font-size:10px;">Fare Breakup</a>
                    </p>
                     </div>
...
<p><?php echo count($guest); ?> room(s) for <?php echo ($numberOfNights!='')?$numberOfNights.' Night':''; ?></p>
				...
				<div class="col-md-12" style="margin-top:20px;padding: 0px;">
					...
					<!--<input type="hidden" id="guest" name="guest" value="">--><label class="add_rooms_plus text_color" style="float: right;font-size: 12px;"><span  style="font-size: 14px;">+</span> Rooms</label>
				 ...
				 <form class="book-hotel-form" method="get" name="hotel_book" action="book.php">
                  ...
                  if(isset($roomNames) && !empty($roomNames)){

                        foreach($roomNames as $roomNamesK => $roomNamesV){
                           foreach($roomNamesV as $withkey => $withval){
                           $idRoomTypeArr = explode('_',$roomNamesK);
                           if($withkey==2)
                           {
                               echo '<option value="'.$roomNamesK.'" '.(($roomNames[$first_key][$first_val]==$roomNames[$roomNamesK][$withkey]) ? 'selected' : '').' data-rate="'.(isset($price1[$roomNamesK][$withkey]) ? $price1[$roomNamesK][$withkey] : '').'" data-id-room="'.$idRoomTypeArr[0].'" data-id-room-type="'.$idRoomTypeArr[1].'" data-cgstamount="'.$withoutgstArrSelect[$roomNamesK]['cgstAmount'].'" data-sgstamount="'.$withoutgstArrSelect[$roomNamesK]['cgstAmount'].'" data-basetarrif="'.$price1[$roomNamesK][$withkey].'">'.$roomNamesV[$withkey].'</option>'; 
                            }else
                            {
                               echo '<option value="'.$roomNamesK.'" '.(($roomNames[$first_key][$first_val]==$roomNames[$roomNamesK][$withkey]) ? 'selected' : '').' data-rate="'.(isset($price1[$roomNamesK][$withkey]) ? $price1[$roomNamesK][$withkey] : '').'" data-id-room="'.$idRoomTypeArr[0].'" data-id-room-type="'.$idRoomTypeArr[1].'" data-cgstamount="'.$gstArrSelect[$roomNamesK]['cgstAmount'].'" data-sgstamount="'.$gstArrSelect[$roomNamesK]['cgstAmount'].'" data-basetarrif="'.$price1[$roomNamesK][$withkey].'">'.$roomNamesV[$withkey].'</option>'; 
                            }
				...
				<div class="col-md-12" style=" margin-top:20px; padding: 0px;<?php if($noRoomsFlag){ ?>display:none;<?php } ?>">
                    ...
                    <a href="#" data-toggle="modal" data-target="#myModalLunch" ><span class="glyphicon glyphicon-info-sign text_color"    style=" position: relative;top: 1px;"></span></a></div>

                    <div class="col-xs-6 " style="padding: 0">
                    <label ><input type="checkbox" value="" class="extraRateDinner" data-type="2" style=" ">Dinner Incl.</label>
                    <a href="#" data-toggle="modal" data-target="#myModalDinner" ><span class="glyphicon glyphicon-info-sign text_color"></span></a></div>
                ...
                <div class="col-md-12" style="text-align:center;margin-top: 20px;<?php if($noRoomsFlag){ ?>display:none;<?php } ?>" id="totalhotel">
                  ...
                  <div class="modal fade" role="dialog" id="myModalFareDetails">
										...
										<p>
                                  		...
                                  		<i class="fa fa-inr" aria-hidden="true"></i>
											<span class="pull-right">
												...
												<span id="cgstRateDiv" ><?php echo $availableRoomsList[0]['cgstAmount']; ?></span>
											</span>
										</p>
										<p><span class="pull-left">Tax SGST</span>
                                  		<i class="fa fa-inr" aria-hidden="true"></i>
											<span class="pull-right">
												...
												<span id="sgstRateDiv"><?php echo $availableRoomsList[0]['sgstAmount']; ?></span>
											</span>
										</p>
										<p><span class="pull-left">Total</span>
                                  		<i class="fa fa-inr" aria-hidden="true"></i>
											<span class="pull-right">
												...
												<span id="totalRateDiv"><?php echo round($totalPriceInitial + $availableRoomsList[0]['cgstAmount'] + $availableRoomsList[0]['sgstAmount']); ?></span>
											</span>
										</p>
                    ...
                    <div class="col-md-12 book_button_hotel">
							...
							<input type="Hidden" name="room_base_rate" id="room_base_rate" value="<?php echo (isset($availableRoomsList[0]['without_baserate'])&&($availableRoomsList[0]['without_baserate']!=0) ? $availableRoomsList[0]['without_baserate'] : (isset($availableRoomsList[0]['baserate']) ? $availableRoomsList[0]['baserate']:'')); ?>">
							<input type="Hidden" name="room_base_tarrif" id="room_base_tarrif" value="<?php echo (isset($availableRoomsList[0]['without_baseTariff'])&&($availableRoomsList[0]['without_baseTariff']!=0) ? $availableRoomsList[0]['without_baseTariff'] : (isset($availableRoomsList[0]['baseTariff']) ? $availableRoomsList[0]['baseTariff']:'')); ?>">
							...
							<input type="Hidden" name="breakfast_incl" id="breakfast_incl" value="<?php echo (isset($availableRoomsList[0]['without_baserate'])&&($availableRoomsList[0]['without_baserate']!='') ? 2 : 1); ?>">
							...
							<input type="Hidden" name="id_property" id="id_property" value="<?php echo (isset($id_property) ? $id_property : ''); ?>">
							 <button type="submit" class=" btn btnBook11 button_color"  value="Book Now">
							 	Book Now
							 </button>
                  ...
                  <ul class="small-list">
                     ...
                     <li>
                        <a href="">
                           ...
                           <p>
                              <span class="curr text_color"><i class="fa fa-inr" aria-hidden="true"></i></span><span class="amount text_color">95</span> per night			
                           </p>
                     ...
                     <li>
                        <a href="">
                           ...
                           <p>
                              <span class="curr text_color"><i class="fa fa-inr" aria-hidden="true"></i></span><span class="amount text_color">115</span> per night			
                           </p>
                     ...
                     <li>
                        <a href="">
                           ...
                           <p>
                              <span class="curr text_color"><i class="fa fa-inr" aria-hidden="true"></i></span><span class="amount text_color">176</span> per night			
                           </p>
                        </a>
                     </li>
                     <li>
                        <a href="">
                           ...
                           <p>
                              <span class="curr text_color"><i class="fa fa-inr" aria-hidden="true"></i></span><span class="amount text_color">215</span> per night			
                           </p>
                        </a>
                     </li>
                  </ul>
            ...
            <table class="extraitems responsive">
               ...
               <tbody>
                  ...
                  <tr>
                     ...
                     <td>
                        <em>
                        <span class="curr text_color">$</span>
                        <span class="amount text_color">30</span>
                  ...
                  <tr>
                     ...
                     <td>
                        <em>
                        <span class="curr text_color">$</span>
                        <span class="amount text_color">1</span>
                  ...
                  <tr>
                     ...
                     <td>
                        <em>
                        <span class="curr text_color">$</span>
                        <span class="amount text_color">10</span>
...
<script type="text/javascript">
    jQuery(document).ready(function($) {
		...
		$('.hotelname').change(function(){
         ...
         $('#baseRateDiv').html(Math.round(baseRate));

      

         var cgstAmount = $(this).find(':selected').attr('data-cgstamount');

         var sgstAmount = $(this).find(':selected').attr('data-sgstamount');

         $('#cgstRateDiv').html(Math.round(parseFloat(cgstAmount)));

         $('#sgstRateDiv').html(Math.round(parseFloat(sgstAmount)));

         $('#totalRateDiv').html(Math.round(baseRate+parseFloat(cgstAmount)+parseFloat(sgstAmount)));

         $('#totalRateSpan').html(Math.round(baseRate+parseFloat(cgstAmount)+parseFloat(sgstAmount)));

         

         var roomBaseTarrif = $(this).find(':selected').attr('data-basetarrif');

         $('#room_base_tarrif').val(parseFloat(roomBaseTarrif));
         ...
         $('.room_type_name').html(($(this).find('option:selected').text()));
      ...
      $('.book_room').click(function(){
         ...
         $('#baseRateDiv').html(Math.round(parseFloat(baseRate)));

         

         var cgstAmount = $(this).attr('data-cgstamount');

         var sgstAmount = $(this).attr('data-sgstamount');
         ...
         var roomBaseTarrif = $(this).attr('data-basetarrif');
