...
$html = '<table style="border:1px solid #808080;line-height:1.5; width:100%;">
    <tr>
        <td>
            ...
            $html .= '<table style=" width:100%;color: #555555;font-size:10px;line-height:1.5; ">';
                <?php 
				$i = 1; $inc=1; $a=0; $pnr="gtest";
					foreach($detailsArr as $pnrKey => $detial){ if($i==1){ $i++;?>
            ...
            $html.='<tr>
                    ...
                    <td rowspan="3" style="font-size: 12px ;text-align:right; border-bottom:1px solid #eee;">
                        '.$_SESSION['agentname'].'
                    </td>
                </tr>

                <tr>
                    <td style="border-bottom:1px solid #eee;"><span style="color:black;">Booking Date on:</span>
                        <?php echo $detial['IssueDate']; ?> </td>
                </tr>';
                <?php } ?>
            ...
            $html.='  <table style=" width:100%;color: #555555; font-size:10px;">

                <tr>
                    <TD> &nbsp;</TD>
                </tr>
                <tr>
                    <TD colspan="2" style="font-size: 14px;margin-bottom: 5px;margin-top: 20px;color: black !important;">';
                        <?php if($inc==1){ echo 'Iterinary and Reservation Details';  $inc++; } else { echo 'Return Journey';} ?>
            $html.='  </TD>
                </tr>
                <tr>
                    <td><span style="color:black;">Booking ID:</span>
                        '. $segmentVal['BTBookingId'].'
                    </td>

                </tr>
                <tr>
                    <td><span style="color:black;">Airline Pnr:</span>
                      '. $pnrKey.'
                    </td>
           ...
           $html.=' <table style="border:1px solid #eee; width:96%; color: #555555; font-size:10px; line-height:1.5;" cellspacing=5>
                <tr>
                    <td colspan="4">&nbsp;</td>
                </tr>
                <tr>
                    ...
                    <td style="text-align: center;">
                         '.$segmentVal['AirlineName'].' '.$segmentKey.'
                    </td>
                    <td style="text-align: center;border-left:1px solid #eee;border-right:1px solid #eee;">Departure<br>
                       '. $segmentVal['segmentOriginCityCode'].'('.(!empty($segmentVal['segmentOriginAirportName']) && strpos($segmentVal['segmentOriginAirportName'],$segmentVal['segmentOriginCityName']) === false ? $segmentVal['segmentOriginAirportName'].', '.$segmentVal['segmentOriginCityName'] : $segmentVal['segmentOriginCityName']).')'.'
                        <b>';

                        <?php if(isset($segmentVal['segmentOriginTerminal']) && !empty($segmentVal['segmentOriginTerminal'])){ ?>Terminal <?php echo $segmentVal['segmentOriginTerminal'];} ?> 
         $html.='</b><br>
                      '. date('l, j M, Y H:i',strtotime($segmentVal['segmentOriginDepTime'])).' 

                 </td>
                    <td style="text-align: center;border-right:1px solid #eee;">Arrival<br>
                        <b>'. $segmentVal['segmentDestinationCityCode'].'('.(!empty($segmentVal['segmentDestinationAirportName']) && strpos($segmentVal['segmentDestinationAirportName'],$segmentVal['segmentDestinationCityName']) === false ? $segmentVal['segmentDestinationAirportName'].', '.$segmentVal['segmentDestinationCityName'] : $segmentVal['segmentDestinationCityName']).')'.'</b><b>';




                        <?php if(isset($segmentVal['segmentDestinationTerminal']) && !empty($segmentVal['segmentDestinationTerminal'])){ ?>Terminal <?php echo $segmentVal['segmentDestinationTerminal'];} ?>
         ...
         $html.='</b><br>
                    ...
                    <td style="text-align: center;">
                        ...
                        Cabin :
                         '.(isset($segmentVal['segmentFareClass']) && !empty($segmentVal['segmentFareClass']) ? $segmentVal['segmentFareClass'].' Class' : '').'
                    </td>

                </tr>
                <tr>
                    <td colspan="1">
                        '. (isset($segmentVal['segmentStopPoint']) && !empty($segmentVal['segmentStopPoint']) ? $segmentVal['segmentStopPoint'].' Stop(s)' : 'Non stop'); .'Flight</td>
                    <td colspan="2">Meal Plan: <b>North Indian meals</b></td>
                    <td colspan="1">Baggages: <b>25kg</b></td>
           			 ...
           			 <?php  $pnr=$pnrKey;} ?>
        $html.='	 <table style="border:1px solid #eee; width:100%;border-top:none;color: #555555;font-size:10px; line-height:1.5; " cellpadding="0" cellspacing="0">
                <tr>
                    <td colspan="8">&nbsp;</td>
        ...
        $html.='   <tr>
                ...
                <tr style="text-align:center;">
                    ...
                    <td style="">
                       '.$passenger['passengerTitle'].' '.$passenger['passengerFirstName'].' '.$passenger['passengerLastName'].'
                    </td>
                    <td style="">';
                        <?php
         ...
         $html.='  </td>
                    <td style="">
                        '. ($passenger['passengerGender'] == 1 ? 'Male' : 'Female').'
                    </td>
                    <td style="">
                        '. $passenger['passengerTicketNumber'].'
                    </td>
                    <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
                </tr>
                <tr>
        ...
        $html.='    </table>;
         ...
         $html.='   <table style="border:1px solid #eee; width:100%;border-top:none;color: #555555;font-size:10px; line-height:1.5;">
                <tr>
                    <td colspan="5">&nbsp;</td>
                </tr>
                <tr>
                    ...
                    <td>
                        Rs. '.number_format($totalBaseFare,2,'.','').'
                    </td>
                ...
                <tr>
                    <td>JnTax</td>
                    <td>
                       Rs. '.number_format($totalJNTax,2,'.','').'
                    </td>
                </tr>
                <tr>
                    <td>Fees & Surcharge</td>
                    <td>
                        Rs. '.number_format($totalFeeSur,2,'.','').'
                    </td>
                </tr>
                <tr>
                    <td>Total Fare</td>
                    <td>
                        Rs. '.number_format(round($totalPublishedFare),2,'.','').'
                    </td>
                </tr>
                ...
                <tr>
                    <td style="font-size:9px;">
                        <ol>
                            ...
                            <li> The conditions may be obtained from the issuing carrier.If the passenger\'s journey involves an ultimate destination or stop in a country other than country of departure the Warsaw convention may be applicable and convention governs and in most case limits the liablity of carriers for death or personal injury and in respect of loss of or damage to baggage</li>
                        </ol>
